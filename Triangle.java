...
class Triangle {
    ...
    Triangle(long a0, long b0, long c0) {
        a = a0; b = b0; c = c0;
    }

    // Classify this triangle from the lengths
    Type classify() {
        if (isIllegal()) return Type.Illegal;
        if (isEquilateral()) return Type.Equilateral;
        if (isFlat()) return Type.Flat;
        if (isIsosceles()) return Type.Isosceles;
        if (isRight()) return Type.Right;
        if (isImpossible()) return Type.Impossible;
    ...
    private boolean check(int a, int b, int c, Type t) {
